...
const fetcher = async (endpoint:string,index:number):Promise<object|boolean> => {
	if(index === 0){
		let serverStatus = false;
		const serverRes = await fetch(endpoint);
		if(serverRes.ok){serverStatus = true;}
		return serverStatus;
	}
	try {
...
function InfiniteScroller({src}:{src:string}){
	...
	const {data,isLoading,isError}:{data:any,isLoading:boolean,isError:boolean} = useQuery(["data",src,latestIndex],()=>fetcher(src,latestIndex),{
		onSuccess:(data:any)=>{
